...
Open PowerShell or Command Prompt in this folder (`c:\Users\Deepak.Tak\Downloads\TEst\Vehicle`).
...
By default, the API key is set to: `your_secret_api_key`.
You can change your secret key:
- **Locally**: Set environment variable `API_KEY="your_custom_key"` (or in [Vehicle.py](file:///c:/Users/Deepak.Tak/Downloads/TEst/Vehicle/Vehicle.py#L13)).
...
- **Methods**: `GET` or `POST`
- **Parameters**:
  - `key` (*required*): Your API key (e.g. `your_secret_api_key`).
  - `quiry` (*required*): Indian vehicle registration number (e.g. `BR05PB3111`, `MH12AB1234`, `DL01A1234`).
...
```
http://localhost:5000/vehicle?key=your_secret_api_key&quiry=BR05PB3111
```

#### PowerShell / curl Example:
```powershell
curl "http://localhost:5000/vehicle?key=your_secret_api_key&quiry=BR05PB3111"
```
...
curl -X POST http://localhost:5000/vehicle `
  ...
  -d '{"key": "your_secret_api_key", "quiry": "BR05PB3111"}'
```
